...
public class MyMapView: NSObject, FlutterPlatformView, CLLocationManagerDelegate, TGMapViewDelegate, TGRecognizerDelegate {
    ...
    var markerIcon: MarkerIconData?
    var personMarkerIcon: MarkerIconData?
    var arrowDirectionIcon: MarkerIconData?
    ...
    var userLocation: MyLocationMarker?
    var dictClusterAnnotation: [String: [StaticGeoPMarker]] = .init()
    var dictIconClusterAnnotation = [String: MarkerIconData]()

    var pickedLocationSingleTap: CLLocationCoordinate2D?
    var roadPicked: RoadFolder?
    var colorRoad: String = "#ff0000"
    var homeMarker: TGMarker?
    var resultFlutter: FlutterResult?
    var methodCall: FlutterMethodCall?
    var uiSingleTapEventMap: UILongPressGestureRecognizer = .init()
    lazy var roadManager: RoadManager = .init()

    let mainView: UIStackView
    var pickerMarker: UIImageView?
    var cacheMarkers: [TGMarker] = .init()

    // var tileRenderer:MKTileOverlayRenderer!

    var stepZoom = 1.0
    var initZoom = 10.0
    var customTiles: [String: Any]?
    var oldCustomTile: CustomTiles?
    var bounds: [Double]?
    ...
    init(_ frame: CGRect, viewId: Int64, channel: FlutterMethodChannel, args: Any?, dynamicOSM: String?, defaultPin: String?) {
        ...
        defaultMarker = MarkerIconData(image: defaultIcon,
    ...
    private func onListenMethodChannel(call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "road":
            drawRoad(call: call) { [unowned self] roadInfo, road, roadData, box, error in
                if error != nil {
                    ...
                    if let bounding = box {
                        mapView.cameraPosition = mapView.cameraThatFitsBounds(bounding, withPadding: UIEdgeInsets(top: 25.0, left: 25.0, bottom: 25.0, right: 25.0))
                    }
        ...
        case "draw#multi#road":
            drawMultiRoad(call: call) { [unowned self] roadInfos, roadsAndRoadData, _ in
    ...
    private func setCameraAreaLimit(call: FlutterMethodCall) {
        ...
        mapView.cameraThatFitsBounds(bounds, withPadding: UIEdgeInsets(top: 3.0, left: 3.0, bottom: 3.0, right: 3.0))
        // mapView.bounds
    ...
    private func zoomMapToBoundingBox(call: FlutterMethodCall) {
        ...
        mapView.cameraPosition = mapView.cameraThatFitsBounds(bounds, withPadding: UIEdgeInsets(top: padding, left: padding, bottom: padding, right: padding))
        // mapView.bounds
        // mapView.bounds = bounds
    }
    ...
    private func changePositionMarker(call: FlutterMethodCall) {
        ...
        var icon = MarkerIconData(image: nil)
        var angle = 0
        var anchor: AnchorGeoPoint?
...
//             }
    ...
    private func drawMultiRoad(call: FlutterMethodCall, completion: @escaping (_ roadsInfo: [RoadInformation?], _ roads: [(String, Road, RoadData)?], Any?) -> Void) {
        let args = call.arguments as! [[String: Any]]
        ...
        group.notify(queue: .main) {
            ...
            for (index, res) in results.enumerated() {
                var roadInfo: RoadInformation?
                var routeToDraw: (String, Road, RoadData)?
                if let road = res.1 {
    ...
    private func drawRoad(call: FlutterMethodCall, completion: @escaping (_ roadInfo: RoadInformation?, _ road: Road?, _ roadData: RoadData?, _ boundingBox: TGCoordinateBounds?, _ error: Error?) -> Void) {
        let args = call.arguments as! [String: Any]
    ...
    private func drawRoadManually(call: FlutterMethodCall, result: FlutterResult) {
        ...
        if zoomInto {
            ...
            mapView.cameraPosition = mapView.cameraThatFitsBounds(box, withPadding: UIEdgeInsets(top: 25.0, left: 25.0, bottom: 25.0, right: 25.0))
        }
        result(nil)
    }
        ...
        if canGetLastUserLocation || canTrackUserLocation {
            if let location = locations.last?.coordinate {
                ...
                if !canSkipFollow {
                    ...
                    mapView.flyToUserLocation(for: location) { [self] _ in
                        // canSkipFollow = true
