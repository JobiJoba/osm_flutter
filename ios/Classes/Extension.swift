...
extension GeoPointMap {
    ...
    func changeIconMarker(on map: TGMapView) {
    ...
    func changePositionMarker(on map: TGMapView, mPosition: CLLocationCoordinate2D) {
        let indexToUpdate = map.markers.firstIndex { m in
            m.point == self.coordinate
        }
        if indexToUpdate != nil {
...
extension TGPolyline {
    ...
    func isOnPath(coordinate: CLLocationCoordinate2D, geodesic: Bool, tolerance: Double = defaultToleranceInMeters) -> Bool {
        ...
        if geodesic {
            for coord in coordinates {
                ...
                if self.isOnSegmentGreatCircle(latLng1: latLng1, latLng2: latLng2, latLng3: targetLatLng, havTolerance: havTolerance) {
                    return true
                }
                latLng1 = latLng2
            }
        ...
        for coord in coordinates {
            ...
            guard max(latLng1.latitude, latLng2.latitude) >= minAcceptable &&
                min(latLng1.latitude, latLng2.latitude) <= maxAcceptable
            else {
                continue
            }
    ...
    private func intersects(
        ...
        return geodesic ?
            tan(latLng3.latitude) >= self.tanLatGreatCircle(lat1: lat1, latLng2: latLng2, lng3: latLng3.longitude) :
            Math.mercatorY(latitudeInRadians: latLng3.latitude) >= self.mercatorLatRhumb(lat1: lat1, latLng2: latLng2, lng3: latLng3.longitude)
    }
    ...
    private func isOnSegmentGreatCircle(
        ...
        let sinBearing = self.sinDeltaBearing(latLng1: latLng1, latLng2: latLng2, latLng3: latLng3)
...
extension Array where Element == RoadFolder {
    func onlyTGMarkerPoylines() -> [TGMarker] {
        let filerRoad = self.filter { roadF in
            roadF.tgRouteLayer.tgMarkerPolyline != nil
        }
        return filerRoad.map { roadF in roadF.tgRouteLayer.tgMarkerPolyline! }
    }
...
extension Array where Element == Int {
    func toUIColor() -> UIColor {
        UIColor(absoluteRed: self.first!, green: self.last!, blue: self[1], alpha: 255)
    }
}
...
extension TGMapView {
    func getBounds(width: CGFloat, height: CGFloat) -> [String: Double] {
        ...
        var latMin: Double?, latMax: Double?, lonMin: Double?, lonMax: Double?
    ...
    func toBounds() -> TGCoordinateBounds {
        let locations: [String: Double] = self.getBounds(width: bounds.width, height: bounds.height)
